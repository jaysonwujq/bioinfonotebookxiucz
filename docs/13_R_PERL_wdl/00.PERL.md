...
- [安装模块](#安装模块)
    - [1. 手工安装](#1-手工安装)
    - [2. CPAN/perl -MCPAN -e 'install 模块'()](#2-cpanperl--mcpan--e-install-模块)
    - [3. cpan shell vs.](#3-cpan-shell-vs)
        - [简单粗暴的初始化cpan，大侠重新来过](#简单粗暴的初始化cpan大侠重新来过)
        - [cpan -i 模块名](#cpan--i-模块名)
- [cpanm专题（！！！）](#cpanm专题)
    - [mcpan安装和配置](#mcpan安装和配置)
    - [其他客户端](#其他客户端)
- [其他问题](#其他问题)
    - [1. PERLLIB and PERL5LIB](#1-perllib-and-perl5lib)
    - [2. conda的perl和系统的perl冲突](#2-conda的perl和系统的perl冲突)
    - [perl 在新版gcc中编译不通过解决办法](#perl-在新版gcc中编译不通过解决办法)
    - [问题](#问题)
    - [深入@INC](#深入inc)
    - [3.1. Can't locate Module/Build.pm in @INC](#31-cant-locate-modulebuildpm-in-inc)
        - [Ref_Info](#ref_info)
    - [3.2 loadable library and perl binaries are mismatched (got handshake key 0xce00080, needed 0xde00080)](#32-loadable-library-and-perl-binaries-are-mismatched-got-handshake-key-0xce00080-needed-0xde00080)
    - [3.3 安装Bio::DB::Sam出错](#33-安装biodbsam出错)
- [遇到问题-------perl无法使用多线程This Perl not built to support threads](#遇到问题-------perl无法使用多线程this-perl-not-built-to-support-threads)
- [安装bioperl](#安装bioperl)
- [perl -V](#perl--v)
...
```
#卸载
/public/frasergen/MED/software/zhangbo/PERL/cpanm --uninstall JSON::XS
...
$ ./Configure -des -Dprefix=`pwd` -Dusethreads -Uversiononly
$ make


1. 由于在新版本中GCC把C99中的标准库分成了libc和libm两个部分，libm中包含一些数学库等，如果要用到libm时，必须加上-lm选项。在解压Configure之后，再在Makefile中，LDFLAGS 和 CLDFLAGS两个参数后面加上 -lm即可解决问题
> 参考：http://serverfault.com/questions/145288/make-error-when-compiling-perl-5-12-1-rhel-5-5

2. 修改 dist/IO/poll.h：将 #include<poll.h> 修改为 #include<sys/poll.h>，注意行数。
```
14c14
< #  include <poll.h>
---
> #  include <sys/poll.h>
```
3. 在Makefile 中，LDFLAGS 和 CLDFLAGS两个参数后面加上 -lrt
...
```
方法一：
cd /public/frasergen/MED/software/PERL/perl-5.32.1/BioPerl-1.6.924
perl Build.PL --install_base=/public/frasergen/MED/software/PERL/perl-5.32.1/BioPerl-1.6.924      
# ./Build install
