...
- [This produces a six-column table as shown. The `alt_count` is the count of reads that support the ALT allele in the germline resource. The `allele_frequency` corresponds to that given in the germline resource. Counts for `other_alt_count` refer to reads that support all other alleles.](#this-produces-a-six-column-table-as-shown-the alt_count is-the-count-of-reads-that-support-the-alt-allele-in-the-germline-resource-the allele_frequency corresponds-to-that-given-in-the-germline-resource-counts-for other_alt_count refer-to-reads-that-support-all-other-alleles)
    - [Mutect2 bamout reports fewer reads than VCF AD](#mutect2-bamout-reports-fewer-reads-than-vcf-ad)
            - [Ref_Info](#ref_info)
    - [方法](#方法)
        - [定义活跃区域](#定义活跃区域)
        - [Assembling Haplotypes](#assembling-haplotypes)
        - [Somatic Genotyping](#somatic-genotyping)
        - [过滤](#过滤)
    - [Results](#results)
        - [DREAM Challenge](#dream-challenge)
        - [Normal-Normal Calls](#normal-normal-calls)
        - [Normal mixtures](#normal-mixtures)
        - [workflow](#workflow)
        - [参数理解](#参数理解)
        - [参数理解](#参数理解-1)
